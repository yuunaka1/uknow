...
export default function Coaching({ geminiApiKey, geminiModel }: { geminiApiKey?: string, geminiModel: string }) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
      setErrorMsg(`ファイルサイズが ${MAX_FILE_SIZE_MB}MB を超えています。音声をMP3に圧縮してください。`);
      setFile(null);
      return;
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg("音声の分析に失敗しました。ファイルが大きすぎるか、形式が無効な可能性があります。");
    } finally {
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '2rem' }}>
      <h2 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', color: 'var(--brand-primary)' }}>
        <Mic2 size={24} /> レッスン音声評価
      </h2>
      <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>
        オンライン英会話のレッスン録音をアップロードしてください。Gemini AIがあなたのスピーキングを分析し、実践的で率直なコーチングフィードバックを提供します。
        (最大 {MAX_FILE_SIZE_MB}MB。25分のレッスンはMP3への変換を推奨します)。
      </p>
      ...
      {isAnalyzing && (
        <div style={{ 
          ...
          <div style={{ marginBottom: '1rem', fontSize: '1.2rem' }}>&gt; AIモデルを初期化中...</div>
          <div style={{ color: 'var(--text-tertiary)' }}>&gt; 音声ストリームをGeminiにアップロード中...</div>
          <div style={{ color: 'var(--text-tertiary)' }}>&gt; 音声ベクトルを処理し、文字起こしを実行中...</div>
          <div style={{ marginTop: '1rem', display: 'inline-block' }} className="blink">_</div>
          <p style={{ marginTop: '1.5rem', fontSize: '0.8rem', color: 'var(--border-color)' }}>ファイルサイズによっては1〜2分かかる場合があります。</p>
        </div>
      )}
